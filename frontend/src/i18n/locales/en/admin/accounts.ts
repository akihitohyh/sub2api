export default {
    accounts: {
      ...
      openai: {
        ...
        excelBPSMihomoDesc: 'Disabled by default. Configure and start the Mihomo pool first; clients must supply a session identity. Healthy sessions keep their exit. Unhealthy exits are probed and cooled down; affected sessions switch after in-flight requests finish. One extra attempt is allowed only before the request was sent. No direct fallback. Idle bindings expire after 30 minutes and reset on restart. Dynamic IP providers must support sticky exits. Turning this off restores the account proxy.',
