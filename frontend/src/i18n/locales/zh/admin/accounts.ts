export default {
    accounts: {
      ...
      openai: {
        ...
        excelBPSMihomoDesc: '默认关闭。需先配置并启动 Mihomo 代理池，并由客户端提供会话标识。健康会话保持原出口；异常节点经过探测和冷却后再使用，受影响会话在进行中的请求结束后自动换出口。仅确认请求尚未发送时额外重试一次，不回退直连。空闲 30 分钟释放绑定，重启后重新分配。动态 IP 供应商须支持粘性出口。关闭后恢复账号原代理。',
