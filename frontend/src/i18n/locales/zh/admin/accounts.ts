export default {
    accounts: {
      ...
      openai: {
        ...
        excelBPSMihomoDesc: '默认关闭。需先在系统设置中配置并启动 Mihomo 代理池。同一会话持续使用同一代理节点，不同会话按负载分配；需要客户端提供会话标识。节点失效时拒绝请求，不换节点、不回退直连。空闲 30 分钟释放绑定，服务重启后重新分配。动态 IP 供应商须支持粘性出口。关闭后恢复账号原代理。',
