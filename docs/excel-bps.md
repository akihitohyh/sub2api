...
- 动态/订阅节点和 IP 管理代理池共用有界探测：一次选路最多检查 3 个候选节点，每个节点的探测与确认合计最多 4 秒，整体仍受 15 秒选路时限约束。探测使用免认证 HTTPS HEAD，不调用模型。
- 单次探测预算耗尽或模型请求发生连接故障后，节点至少冷却 15 秒，避免被其他会话立即重复选中。连续失败和断流保留更长的原有冷却；恢复仍需两次连续成功探测。客户端取消或自身截止时间到达，不计作节点故障。
